...
"""
    ...
    for item in history:
        ...
        def sort_and_join(links):
            if not links: return ""
            links.sort()
            return "<br>".join(links)

        win_str = sort_and_join(assets_grouped["Windows"])
        mac_str = sort_and_join(assets_grouped["macOS"])
        linux_str = sort_and_join(assets_grouped["Linux"])
        other_str = sort_and_join(assets_grouped["Other"])
        
        md += f"| `{tag}` | {date_bj} | {win_str} | {mac_str} | {linux_str} | {other_str} | [Source]({url}) |\n"
