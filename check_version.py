...
def 生成版本详情卡片(item, title_prefix=""):
    ...
    md += f"**发布时间 (UTC+8)**: `{bj_date}`\n\n"
    
    if assets_grouped["Windows"]:
        md += "**Windows**:\n" + " / ".join(assets_grouped["Windows"]) + "\n\n"
    if assets_grouped["macOS"]:
        md += "**macOS**:\n" + " / ".join(assets_grouped["macOS"]) + "\n\n"
    if assets_grouped["Linux"]:
        md += "**Linux**:\n" + " / ".join(assets_grouped["Linux"]) + "\n\n"
    if assets_grouped["Other"]:
        md += "**Other**:\n" + " / ".join(assets_grouped["Other"]) + "\n\n"
...
"""
    ...
    if stable_version:
        md += "## 🛡️ 最稳定版本 (Most Stable)\n"
        md += 生成版本详情卡片(stable_version, "💎")
    
    md += """## 📜 所有历史版本 (All Versions)
| 版本 (Version) | 发布时间 (UTC+8) | 系统/资源 (Assets) | 原始链接 |
| :--- | :--- | :--- | :--- |
"""
    ...
    for item in history:
        ...
        all_links = []
        if assets_grouped["Windows"]: all_links.extend(assets_grouped["Windows"])
        if assets_grouped["macOS"]: all_links.extend(assets_grouped["macOS"])
        if assets_grouped["Linux"]: all_links.extend(assets_grouped["Linux"])
        if assets_grouped["Other"]: all_links.extend(assets_grouped["Other"])
        
        # 限制每行显示的资源数量，避免表格过宽，使用 <br> 换行
        assets_text = "<br>".join(all_links) if all_links else "No Assets"
        
        md += f"| `{tag}` | {date_bj} | {assets_text} | [Source]({url}) |\n"
